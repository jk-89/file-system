...
// Find number of common files in path prefixes.
size_t common_files(const char *path1, const char *path2);

// Count number of slashes in path (useful while counting files on path).
size_t count_slashes(const char *path);
